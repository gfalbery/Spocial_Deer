...
CentralityRemovalList <- list()

Covar <- c("Age", "ReprodStatus", "Year", "PopN", "NObs")

FCovar <- c("f(Name, model = 'iid')",
            "f(fYear, model = 'iid')" ,
...
FullCovar <- c(Covar, FCovar)

dir_create("Output Files")

CentralityRemovalList <- MeshList <- SPDEList <- TestDFList <- list()
...
FullCovar %>% map(~.x %>% 
                    setdiff(FullCovar, .) %>% 
                    paste(collapse = " + ") %>% str_replace_all("ReprodStatus", 
                                                                "ReprodStatusSummer.Yeld + ReprodStatusWinter.Yeld + ReprodStatusMilk") %>%
                    paste("y ~ -1 + Intercept + ", .) %>% 
                    as.formula) ->
  FormulaList
...
for(r in r:length(SocResps)){
  ...
  for(i in i:length(FormulaList)){
    ...
    CentralityRemovalList[[SocResps[r]]][[i]] <-
      inla(
        FormulaList[[i]], 
        family = FamilyList[r],
  ...
  saveRDS(CentralityRemovalList, file = "Output Files/CentralityRemovalList.rds")
  
}

saveRDS(CentralityRemovalList, file = "Output Files/CentralityRemovalList.rds")

CentralityRemovalList <- readRDS("Output Files/CentralityRemovalList.rds")
